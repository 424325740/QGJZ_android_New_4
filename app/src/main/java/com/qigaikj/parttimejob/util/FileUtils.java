...
public class FileUtils {
     ...
     * 时间命名
     */
    public static final String TIME_STRING = "yyyyMMdd_HHmmss";
    /**
     * 限制图片最大宽度进行压缩
     */
     ...
     * 缓存文件根目录名
     */
    public static final String FILE_DIR = "you";
    /**
     * 上传的照片文件路径
     */
    ...
    public static String getCacheDirPath(Context context) {
        if (isSDCardExist()) {
            String path = MemoryUtils.Path.JUXIN_ROOT_DIRECTORY;
     ...
     * @return
     */
    public static String getUploadVideoCachePath(Context context) {
        if (isSDCardExist()) {
     ...
     * @return
     */
    public static String getUploadImageCachePath(Context context) {
        if (isSDCardExist()) {
            String path=MemoryUtils.Path.JUXIN_IMAGE_DIRECTORY;
            File directory = new File(path);
            if (!directory.exists()) directory.mkdirs();
            return path;
        } else {
    ...
    public static String getUploadPhotoFile(Context context) {
        return getUploadImageCachePath(context)+ getTimeString() + ".jpg";
    }
    ...
    public static String getUploadVideoFile(Context context) {
        return getUploadVideoCachePath(context) + getTimeString() + ".mp4";
    }
