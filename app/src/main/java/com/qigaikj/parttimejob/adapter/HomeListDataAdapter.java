...
public class HomeListDataAdapter extends RecyclerView.Adapter<HomeListDataAdapter.ViewHolder> {
    ...
    public List<HomeYueTa> datas = null;
    public Context context;
    private ItemClickCallBack clickCallBack;

    public HomeListDataAdapter(Context context, List<HomeYueTa> datas) {
    ...
    public void onBindViewHolder(ViewHolder viewHolder,final int position) {
        Glide.with(context).load(datas.get(position).logo).transform(new GlideRoundTransform(context)).into((ImageView) viewHolder.iv_UserLogo);
        viewHolder.tv_UserName.setText(datas.get(position).uname);
        if ("1".equals(datas.get(position).gender)) {
        ...
        if ("1".equals(datas.get(position).state)) {
