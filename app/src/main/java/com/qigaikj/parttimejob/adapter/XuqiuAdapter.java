...
public class XuqiuAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<List<MyXuqiuBean.DataBean>> skillData;
    private Context context;
    private ItemClickCallBack clickCallBack;
    private OnItemLongClickListener mOnItemLongClickListener;
    public interface ItemClickCallBack{
        void onItemClick(int pos,  String kid);
    }
    ...
    public XuqiuAdapter(List<List<MyXuqiuBean.DataBean>> skillData, Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = View.inflate(context, R.layout.activity_xuqiu_item, null);
...
//        Glide.with(context).load(skillData.get(position).pic).into(myViewHolder.ivSkillPicture);
        myViewHolder.tvSkillTitle.setText(skillData.get(position).get(position).getName());
//        myViewHolder.tvSkillTime.setText(skillData.get(position).get(position).addtime);
        myViewHolder.tvSkillIntroduce.setText(skillData.get(position).get(position).getRequire());
        if ("1".equals(skillData.get(position).get(position).getServe())) {
      ...
      /*  if ("1".equals(skillData.get(position).unit)) {
        ...
        if ("1".equals(skillData.get(position).get(position).getStat())) {
        ...
        myViewHolder.switch_button.setOnCheckedChangeListener(new SwitchButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(SwitchButton view, boolean isChecked) {
                if (isChecked) {
                    PostCloseSkill(skillData.get(position).get(position).getKid());
        ...
        myViewHolder.tvEdit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(clickCallBack != null){
                    clickCallBack.onItemClick(position,skillData.get(position).get(position).getKid());
                }
            }
        });
        myViewHolder.delete.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                mOnItemLongClickListener.onItemLongClick(view,position,skillData.get(position).get(position).getKid());
                return false;
            }
        });
    }
    ...
    public void PostCloseSkill(String kid) {
        ...
        RetrofitManager.getInstance().createReq(BlogService.class).postCloseXuqiu(token, kid).enqueue(new Callback<WrapperRspEntity<SkillCloseState>>() {
            @Override
