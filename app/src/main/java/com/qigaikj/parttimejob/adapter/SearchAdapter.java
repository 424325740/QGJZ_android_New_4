...
public class SearchAdapter extends RecyclerView.Adapter<SearchAdapter.ViewHolder> {
    ...
    public List<HomeSearchYuetaBean.DataBean> datas = null;
    public Context context;
    private ItemClickCallBack clickCallBack;

    public SearchAdapter(Context context, List<HomeSearchYuetaBean.DataBean> datas) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.searchjianzhiitem,viewGroup,false);
        return new ViewHolder(view);
    }
    //将数据与界面进行绑定的操作
    @Override
    public void onBindViewHolder(ViewHolder viewHolder,final int position) {
        Glide.with(context).load(datas.get(position).getSkill().get(position).getLogo()).transform(new GlideRoundTransform(context)).into((ImageView) viewHolder.iv_UserLogo);
        viewHolder.tv_UserName.setText(datas.get(position).getSkill().get(position).getUname());
        String state = datas.get(position).getSkill().get(position).getState();
        if (state.equals("1")){
        ...
        if (datas.get(position).getSkill().get(position).getSkill_pic() != null && datas.get(position).getSkill().get(position).getSkill_pic().size() > 0) {
            ...
            viewHolder.rvUserPic.setAdapter(new HomeSearchDataAdapter(context, datas, position));
        } else {
